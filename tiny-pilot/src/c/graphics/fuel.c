...
extern uint8_t fuel_vram_offset; // Need to grab the offset of the fuel bar tilesheet
...
void draw_fuel_bar(GBC_Graphics *graphics, uint16_t fuel_amount, uint16_t max_fuel, uint8_t num_fuel_bars, uint8_t x, uint8_t y) {
    // Change the color of the bar based on the amount of fuel remaining
    ...
    // Calculate the number of pixels that should be filled with fuel
    ...
    // First, fill up all of the full bars
    ...
    // Then, fill up the bar that is somewhere between full and empty
    ...
    // Next, fill up all of the empty bars
    if (bar_pos < num_fuel_bars) {
    ...
    // And finally, place the end cap onto the fuel bar
