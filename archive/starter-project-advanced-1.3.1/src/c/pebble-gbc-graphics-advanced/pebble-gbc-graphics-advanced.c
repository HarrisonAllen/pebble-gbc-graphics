    ...
    for (self->line_y = 0; self->line_y < self->screen_height; self->line_y++) {
        ...
        for(x = min_x; x < max_x; x++) {
            ...
            for (bg_num = background_start; bg_num > sprite_layer_z; bg_num--) {
                ...
                flip_x = (tile_attr & GBC_ATTR_FLIP_FLAG_X) >> 5; // Flip becomes 1
                pixel_x = pixel_x + (BOOL_MASK[flip_x] & (GBC_TILE_WIDTH - (pixel_x << 1) - 1)); // pixel_x = flip_x ? sprite_w - pixel_x - 1 : pixel_x
                flip_y = (tile_attr & GBC_ATTR_FLIP_FLAG_Y) >> 6; // Flip becomes 1
            ...
            if (self->lcdc & GBC_LCDC_SPRITE_ENABLE_FLAG) {
                for (sprite_id = GBC_NUM_SPRITES - 1; sprite_id >= 0; sprite_id--) {
                    ...
                    // TODO: Somewhere along here y flip doesn't work
                    // Apply flip flags
