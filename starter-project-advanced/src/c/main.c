...
static void create_palettes() {
    GBC_Graphics_set_bg_palette(s_gbc_graphics, 0, 1, 0b11000000);
...
static void generate_sprite() {
    GBC_Graphics_oam_set_sprite(s_gbc_graphics, sprite_num, sprite_min, 30 + Y_OFFSET + GBC_SPRITE_OFFSET_Y, 0, GBC_Graphics_attr_make(4, 0, false, false, false), 0, 0, 0, 0);
}

static void step() {
    sprite_width += 1;
    if (sprite_width > GBC_SPRITE_MAX_WIDTH) {
        sprite_width = 0;
        sprite_height += 1;
        if (sprite_height > GBC_SPRITE_MAX_HEIGHT) {
            sprite_height = 0;
        }
    }
